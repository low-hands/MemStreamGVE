...
class StreamingTrainingPipeline:
    def __init__(self,
        ...
        self.record_interval = kwargs.get("record_interval", 3)
    ...
    def generate_chunk_with_cache(
        ...
        *,
        ...
        update_bank_init = update_bank
        for block_index, current_num_frames in enumerate(all_num_frames):
            if update_bank_init:
                # if block_index%3==0:
                if (current_start_frame + local_start_frame)%(3*self.record_interval)==0:
                    update_bank=True
                else:
                    update_bank=False
            if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                print(f"[SeqTrain-Pipeline] Processing block {block_index}: update_bank: {update_bank}, frames {local_start_frame}-{local_start_frame + current_num_frames}")
            ...
            for step_idx, current_timestep in enumerate(self.denoising_step_list):
                ...
                if step_idx==0:
                    ...
                    q_bank=False
                if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                    print('Current timestep:', step_idx, q_bank)
    ...
    def _initialize_kv_bank(self, batch_size, dtype, device):
        ...
        kv_bank1 = []
        if DEBUG:
            print(f"rank {dist.get_rank()} initialize kv bank with batch_size: {batch_size}, kv_bank_size: {self.kv_bank1_size}")
        for _ in range(self.num_transformer_blocks):
            kv_bank1.append({
