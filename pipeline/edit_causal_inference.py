...
class EditCausalInferencePipeline(torch.nn.Module):
    ...
    def inference(
        ...
        # Build the (optional) ΔKV transport probe ONCE per inference call so its
        # dump store accumulates across all blocks. shared_dict_dual is recreated
        # per block, so we cannot let the probe be rebuilt from it each time.
        from wan.modules.transport_probe import TransportProbe
        probe_obj = None
        if probe_cfg and probe_cfg.get("mode", "off") != "off":
            probe_obj = TransportProbe(
                mode=probe_cfg.get("mode", "off"),
                layers=probe_cfg.get("layers", None),
                save_path=probe_cfg.get("save_path", None),
                capsule_path=probe_cfg.get("capsule_path", None),
                topk=probe_cfg.get("topk", 4),
                temp=probe_cfg.get("temp", 0.07),
                max_per_key=probe_cfg.get("max_per_key", 512),
            )
        ...
        for current_num_frames in tqdm(all_num_frames):
            ...
            self._log_bank_slot_alignment(
                kv_bank_dual,
                block_start=current_start_frame,
                log_path=bank_slot_align_log,
            )
            ...
            for index, current_timestep in tqdm(enumerate(denoising_step_list), total=len(denoising_step_list), leave=False):
                ...
                shared_dict_dual['bank_prev_context_mode'] = bank_prev_context_mode
                shared_dict_dual['bank_aux_alpha'] = bank_aux_alpha
                shared_dict_dual['bank_gate_alpha'] = bank_gate_alpha
                shared_dict_dual['bank_gate_tau'] = bank_gate_tau
                shared_dict_dual['bank_gate_log'] = bank_gate_log
                shared_dict_dual['bank_topk'] = bank_topk
                ...
                shared_dict_dual['probe_cfg'] = probe_cfg
                shared_dict_dual['probe'] = probe_obj
        ...
        # ΔKV transport probe: persist dumped capsules after all blocks (dump mode).
        if probe_obj is not None and probe_obj.is_dump:
            probe_obj.save()
    ...
    def _initialize_kv_bank(self, batch_size, dtype, device, kv_bank_size):
        ...
        for _ in range(self.num_transformer_blocks):
            kv_bank.append({
                ...
                "global_end_index": torch.tensor([0], dtype=torch.long, device=device),
                "local_end_index": torch.tensor([0], dtype=torch.long, device=device),
                # MemFlow contract: `k_new` must be materialized into the bank
                # exactly once per block (native inference passes q_bank=True a
                # single time per block). The edit pipeline triggers the target
                # materialization twice -- once explicitly before the dual pass
                # and once via the clean target forward -- which appended the
                # same block twice and evicted an extra real history slot.
                # This flag makes the materialization idempotent.
                "k_new_pending": torch.zeros([1], dtype=torch.bool, device=device),
            })
        return kv_bank
    ...
    def _log_bank_slot_alignment(self, kv_bank, block_start, log_path):
        if log_path is None:
            return
        if not kv_bank or "k" not in kv_bank[0]:
            return
        ...
        layers = sorted({0, len(kv_bank) // 2, len(kv_bank) - 1})
        with open(log_path, "a") as f:
            for layer_idx in layers:
                ...
                src_k, trg_k = k.chunk(2, dim=0)
                valid_src = src_k.abs().sum(dim=(2, 3)) > 0
                valid_trg = trg_k.abs().sum(dim=(2, 3)) > 0
                valid = valid_src & valid_trg

                if valid.numel() == 0 or not valid.any():
                    f.write(
                        f"[BANK_SLOT] block={block_start} layer={layer_idx} "
                        f"valid=0 cos_mean=nan cos_p10=nan cos_p50=nan cos_p90=nan "
                        f"fg_match=nan fg_iou=nan\n"
                    )
                    continue

                src_flat = src_k.flatten(2).float()
                trg_flat = trg_k.flatten(2).float()
                cos = torch.nn.functional.cosine_similarity(src_flat, trg_flat, dim=-1)
                cos_valid = cos[valid]

                fg_match = float("nan")
                fg_iou = float("nan")
                if "fg_mask" in bank:
                    src_fg, trg_fg = bank["fg_mask"].detach().chunk(2, dim=0)
                    src_fg = src_fg[:, :valid.shape[1]]
                    trg_fg = trg_fg[:, :valid.shape[1]]
                    fg_valid = valid
                    if fg_valid.any():
                        fg_match = (src_fg[fg_valid] == trg_fg[fg_valid]).float().mean().item()
                        inter = (src_fg & trg_fg & fg_valid).sum().float()
                        union = ((src_fg | trg_fg) & fg_valid).sum().float()
                        fg_iou = (inter / (union + 1e-6)).item()

                qs = torch.quantile(
                    cos_valid,
                    torch.tensor([0.1, 0.5, 0.9], device=cos_valid.device),
                )

                f.write(
                    f"[BANK_SLOT] block={block_start} layer={layer_idx} "
                    f"valid={int(valid.sum().item())} "
                    f"cos_mean={cos_valid.mean().item():.6f} "
                    f"cos_p10={qs[0].item():.6f} "
                    f"cos_p50={qs[1].item():.6f} "
                    f"cos_p90={qs[2].item():.6f} "
                    f"fg_match={fg_match:.6f} "
                    f"fg_iou={fg_iou:.6f}\n"
                )
