...
if __name__ == '__main__':
    ...
    parser.add_argument("--bank_context_mode", type=str, default="normal", choices=["normal", "target_fg_source_bg", "confident_fg"], help="Bank design: use target bank for foreground tokens and source bank for background tokens.")
    parser.add_argument("--bank_repeat_factor", type=int, default=1, help="Bank usage ablation: repeat memory-bank KV in dual attention.")
    parser.add_argument("--bank_prev_context_mode", type=str, default="normal", choices=["normal", "bank_only", "no_bank"], help="Diagnostic: choose which previous-context KV target branch can use.")
    parser.add_argument("--bank_aux_alpha", type=float, default=0.0, help="Soft auxiliary bank attention weight for target branch.")
    parser.add_argument("--bank_gate_alpha", type=float, default=0.0, help="Reliability-gated bank residual strength.")
    parser.add_argument("--bank_gate_tau", type=float, default=0.15, help="Reliability gate temperature. Smaller means stricter bank usage.")
    parser.add_argument("--bank_gate_log", type=str, default=None, help="Optional path to log bank reliability gate statistics.")
    parser.add_argument("--bank_topk", type=int, default=0, help="Use top-k retrieved bank tokens for gated bank attention. 0 means use all bank tokens.")
    parser.add_argument("--bank_debug_log", type=str, default=None, help="Optional path for source/target bank alignment diagnostics.")
    parser.add_argument("--bank_slot_align_log", type=str, default=None, help="Optional path for slot-level source/target bank alignment diagnostics.")
    parser.add_argument("--disable_bank_in_dual", action="store_true", default=False, help="Ablation: disable MemFlow bank during dual-branch denoising.")
    parser.add_argument("--attn_mass_log", type=str, default=None, help="Optional path for target self-attention mass over sink/bank/local.")
    parser.add_argument("--debug_mask_dir", type=str, default=None, help="Optional directory to dump cross-attention edit mask overlays.")
    parser.add_argument("--debug_velocity_mask_dir", type=str, default=None, help="Optional directory to dump velocity-difference edit mask overlays.")
    parser.add_argument(
        "--bank_memory_mode",
        type=str,
        default="normal",
        choices=["normal", "drop_source", "drop_src_bank"],
        help="Bank-specific ablation: drop source bank contribution when target branch reads bank memory.",
    )
    parser.add_argument("--edit_mask_mode", type=str, default="velocity", choices=["velocity", "text_velocity"])
    parser.add_argument("--edit_mask_text_power", type=float, default=1.0)
    parser.add_argument("--edit_mask_threshold", type=float, default=None)
    # ---- ΔKV Transport Probe (EXPERIMENT_PLAN Block 1) ----
    parser.add_argument("--probe_mode", type=str, default="off",
                        choices=["off", "dump", "C0", "C1", "C2"],
                        help="ΔKV transport probe mode: dump shot A capsules, or C0/C1/C2 inject on shot B.")
    parser.add_argument("--probe_save_path", type=str, default=None,
                        help="Where to write capsules at end of a dump run (.pt).")
    parser.add_argument("--probe_capsule_path", type=str, default=None,
                        help="Capsule .pt to load for C1/C2 inject runs.")
    parser.add_argument("--probe_layers", type=str, default=None,
                        help="Comma-separated or 'a-b' layer range to restrict probe (e.g. '12-26'). None=all.")
    parser.add_argument("--probe_topk", type=int, default=4)
    parser.add_argument("--probe_temp", type=float, default=0.07)
    parser.add_argument("--probe_max_per_key", type=int, default=512)
    ...
    # Build ΔKV transport probe config (Block 1). off => no-op everywhere.
    probe_layers = None
    if args.probe_layers:
        if "-" in args.probe_layers and "," not in args.probe_layers:
            _a, _b = args.probe_layers.split("-")
            probe_layers = list(range(int(_a), int(_b) + 1))
        else:
            probe_layers = [int(x) for x in args.probe_layers.split(",") if x.strip()]
    probe_cfg = {
        "mode": args.probe_mode,
        "save_path": args.probe_save_path,
        "capsule_path": args.probe_capsule_path,
        "layers": probe_layers,
        "topk": args.probe_topk,
        "temp": args.probe_temp,
        "max_per_key": args.probe_max_per_key,
    }
    ...
    edit_video = pipeline.inference(
        ...
        bank_debug_log=args.bank_debug_log,
        bank_slot_align_log=args.bank_slot_align_log,
        disable_bank_in_dual=args.disable_bank_in_dual,
        attn_mass_log=args.attn_mass_log,
        debug_mask_dir=args.debug_mask_dir,
        debug_velocity_mask_dir=args.debug_velocity_mask_dir,
        bank_memory_mode=args.bank_memory_mode,
        bank_context_mode=args.bank_context_mode,
        bank_repeat_factor=args.bank_repeat_factor,
        bank_prev_context_mode=args.bank_prev_context_mode,
        bank_aux_alpha=args.bank_aux_alpha,
        bank_gate_alpha=args.bank_gate_alpha,
        bank_gate_tau=args.bank_gate_tau,
        bank_gate_log=args.bank_gate_log,
        bank_topk=args.bank_topk,
        edit_mask_mode=args.edit_mask_mode,
        edit_mask_text_power=args.edit_mask_text_power,
        edit_mask_threshold=args.edit_mask_threshold,
        probe_cfg=probe_cfg,
    )
