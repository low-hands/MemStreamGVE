...
from wan.modules.transport_probe import build_probe_from_shared_dict
...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # ---- ΔKV Transport Probe (EXPERIMENT_PLAN Block 1) ----------------
        # Optional, OFF by default (zero overhead unless probe_cfg.mode != off).
        # Operates in PRE-RoPE K/V space on the dual NOISY forward only, which
        # is uniquely identified by `current_src_fg_mask` living on kv_cache.
        # Batch layout for the dual forward is [src(real_b) || trg(real_b)] and
        # the current-block token dim `s` aligns 1:1 with current_src_fg_mask.
        #   dump : record fg (K_src,V_src,K_tgt,V_tgt) keyed by (layer, bucket)
        #   C1/C2: overwrite trg-branch fg K/V with reconstructed K_hat,V_hat
        #          BEFORE causal_rope_apply, so the residual stays
        #          position-agnostic and is re-roped at the current shot.
        if (
            kv_cache is not None
            and isinstance(kv_cache, dict)
            and kv_cache.get("current_src_fg_mask", None) is not None
        ):
            _sd = kv_cache.get("shared_dict", None)
            _probe = build_probe_from_shared_dict(_sd) if _sd is not None else None
            if _probe is not None and _probe.active:
                _real_b = b // 2
                _bucket = int(_sd.get("current_timestep_index", 0))
                _lid = getattr(self, "layer_idx", -1)
                _fg = kv_cache["current_src_fg_mask"]  # [real_b, s]
                with torch.no_grad():
                    for _bi in range(_real_b):
                        _m = _fg[_bi]
                        _m = _m > 0.5 if _m.dtype != torch.bool else _m
                        # mask must align with current-block token dim of k
                        if _m.shape[0] != k.shape[1]:
                            continue
                        if _probe.is_dump:
                            _probe.record(
                                _lid, _bucket,
                                k[_bi][_m], v[_bi][_m],
                                k[_real_b + _bi][_m], v[_real_b + _bi][_m],
                            )
                        elif _probe.is_inject:
                            _rec = _probe.reconstruct(
                                _lid, _bucket, k[_bi][_m], v[_bi][_m]
                            )
                            if _rec is not None:
                                _k_hat, _v_hat = _rec
                                k[_real_b + _bi][_m] = _k_hat
                                v[_real_b + _bi][_m] = _v_hat
        # ------------------------------------------------------------------
        ...
        if kv_cache is None:
            ...
            if sink_tokens > 0:
                ...
                if kv_cache.get("trg_fg_mask", None) is None or kv_cache.get("current_src_fg_mask", None) is None:
                    ...
                    bank_memory_mode = kv_cache['shared_dict'].get('bank_memory_mode', 'normal')
                    bank_context_mode = kv_cache['shared_dict'].get('bank_context_mode', 'normal')
                    bank_repeat_factor = int(kv_cache['shared_dict'].get('bank_repeat_factor', 1))
                    bank_prev_context_mode = kv_cache['shared_dict'].get('bank_prev_context_mode', 'normal')
                    bank_aux_alpha = float(kv_cache['shared_dict'].get('bank_aux_alpha', 0.0))
                    bank_gate_alpha = float(kv_cache['shared_dict'].get('bank_gate_alpha', 0.0))
                    bank_gate_tau = float(kv_cache['shared_dict'].get('bank_gate_tau', 0.15))
                    bank_topk = int(kv_cache['shared_dict'].get('bank_topk', 0))

                    src_bank_key_for_context = src_bank_key
                    src_bank_value_for_context = src_bank_value
                    trg_bank_key_for_context = trg_bank_key
                    trg_bank_value_for_context = trg_bank_value

                    if (
                        bank_context_mode == "target_fg_source_bg"
                        and bank_memory_mode != "drop_src_bank"
                        and trg_bank_key.shape[1] > 0
                    ):
                        bank_fg_mask = kv_bank.get("fg_mask", None)
                        if bank_fg_mask is not None:
                            _, trg_bank_fg_mask_for_select = bank_fg_mask[:, :trg_bank_key.shape[1]].chunk(2, dim=0)
                            bank_select_mask = trg_bank_fg_mask_for_select[:, :, None, None]
                            trg_bank_key_for_context = torch.where(bank_select_mask, trg_bank_key, src_bank_key)
                            trg_bank_value_for_context = torch.where(bank_select_mask, trg_bank_value, src_bank_value)

                    if bank_repeat_factor > 1 and trg_bank_key_for_context.shape[1] > 0:
                        src_bank_key_for_context = src_bank_key_for_context.repeat_interleave(bank_repeat_factor, dim=1)
                        src_bank_value_for_context = src_bank_value_for_context.repeat_interleave(bank_repeat_factor, dim=1)
                        trg_bank_key_for_context = trg_bank_key_for_context.repeat_interleave(bank_repeat_factor, dim=1)
                        trg_bank_value_for_context = trg_bank_value_for_context.repeat_interleave(bank_repeat_factor, dim=1)

                    src_context_key = torch.cat([src_sink_key, src_bank_key_for_context, src_local_key], dim=1)
                    src_context_value = torch.cat([src_sink_value, src_bank_value_for_context, src_local_value], dim=1)
                    trg_context_key = torch.cat([trg_sink_key, trg_bank_key_for_context, trg_local_key], dim=1)
                    trg_context_value = torch.cat([trg_sink_value, trg_bank_value_for_context, trg_local_value], dim=1)
                    ...
                    if src_sink_key.shape[1] > 0:
                        ...
                        bank_layout_masks.append(
                            torch.zeros([src_sink_key.shape[1]], dtype=torch.bool, device=roped_query.device)
                        )
                    if src_bank_key_for_context.shape[1] > 0:
                        layout_masks.append(
                            torch.zeros([src_bank_key_for_context.shape[1]], dtype=torch.bool, device=roped_query.device)
                        )
                        bank_layout_masks.append(
                            torch.ones([src_bank_key_for_context.shape[1]], dtype=torch.bool, device=roped_query.device)
                        )
                    if src_local_key.shape[1] > 0:
                        ...
                        bank_layout_masks.append(
                            torch.zeros([src_local_key.shape[1]], dtype=torch.bool, device=roped_query.device)
                        )
                    if layout_masks:
                        context_current_mask = torch.cat(layout_masks, dim=0)
                        context_bank_mask = torch.cat(bank_layout_masks, dim=0)
                    else:
                        context_current_mask = torch.zeros(
                            [0], dtype=torch.bool, device=roped_query.device
                        )
                        context_bank_mask = torch.zeros(
                            [0], dtype=torch.bool, device=roped_query.device
                        )
                    if (
                    ...
                    context_prev_bank_mask = context_bank_mask[context_prev_mask]
                    ...
                    if trg_bank_key_for_context.shape[1] > 0:
                        bank_fg_mask = kv_bank.get("fg_mask", None)
                        if bank_fg_mask is not None:
                            _, trg_bank_fg_mask = bank_fg_mask[:, :trg_bank_key.shape[1]].chunk(2, dim=0)
                            if bank_repeat_factor > 1 and trg_bank_fg_mask.shape[1] > 0:
                                trg_bank_fg_mask = trg_bank_fg_mask.repeat_interleave(bank_repeat_factor, dim=1)
                        else:
                            trg_bank_fg_mask = torch.zeros(
                                [trg_bank_key_for_context.shape[0], trg_bank_key_for_context.shape[1]],
                                dtype=torch.bool,
                                device=trg_bank_key_for_context.device,
                            )
                    ...
                    if bank_prev_context_mode in {"bank_only", "no_bank"} and context_prev_bank_mask.shape[0] > 0:
                        if bank_prev_context_mode == "bank_only":
                            prev_keep_mask = context_prev_bank_mask
                        else:
                            prev_keep_mask = ~context_prev_bank_mask

                        src_prev_key = src_prev_key[:, prev_keep_mask]
                        src_prev_value = src_prev_value[:, prev_keep_mask]
                        trg_prev_key = trg_prev_key[:, prev_keep_mask]
                        trg_prev_value = trg_prev_value[:, prev_keep_mask]
                        trg_prev_fg_mask = trg_prev_fg_mask[:, prev_keep_mask]
                        context_prev_bank_mask = context_prev_bank_mask[prev_keep_mask]
                    ...
                    for b_idx in range(b // 2):
                        ...
                        if trg_prev_key.shape[1] > 0:
                            ...
                            if b_trg_prev_fg_mask.any():
                                bank_blend_mask = b_trg_prev_fg_mask
                                if bank_memory_mode in {"drop_source", "drop_src_bank"} or bank_context_mode == "target_fg_source_bg":
                                    bank_blend_mask = b_trg_prev_fg_mask & ~context_prev_bank_mask
                                if bridge_mode == "soft_fg_target":
                                    fg_rate = max(blender_rate, bridge_fg_target_floor)
                                    if bank_blend_mask.any():
                                        b_trg_prev_key[bank_blend_mask] = (
                                            b_trg_prev_key[bank_blend_mask] * fg_rate
                                            + src_prev_key[b_idx, bank_blend_mask] * (1 - fg_rate)
                                        )
                                else:
                                    if bank_blend_mask.any():
                                        b_trg_prev_key[bank_blend_mask] = (
                                            b_trg_prev_key[bank_blend_mask] * blender_rate
                                            + src_prev_key[b_idx, bank_blend_mask] * (1 - blender_rate)
                                        )
                        ...
                        attn_mass_log = kv_cache["shared_dict"].get("attn_mass_log", None)
                        if attn_mass_log and b_idx == 0:
                            with torch.no_grad():
                                q_dbg = b_query.float()
                                k_dbg = b_trg_key.float()

                                sample_q = min(16, q_dbg.shape[0])
                                if q_dbg.shape[0] > sample_q:
                                    q_idx = torch.linspace(
                                        0, q_dbg.shape[0] - 1, sample_q,
                                        device=q_dbg.device
                                    ).long()
                                    q_dbg = q_dbg.index_select(0, q_idx)

                                scores_dbg = torch.einsum("qhd,khd->hqk", q_dbg, k_dbg) * (q_dbg.shape[-1] ** -0.5)
                                probs_dbg = scores_dbg.softmax(dim=-1).mean(dim=(0, 1))

                                prev_len = trg_prev_key.shape[1]
                                cur_len = trg_current_key.shape[1] if can_blend_current else b_trg_current_key.shape[0]
                                prev_bank_mask = context_prev_bank_mask
                                prev_sink_local_mask = ~prev_bank_mask

                                prev_mass = probs_dbg[:prev_len]
                                bank_mass = prev_mass[prev_bank_mask].sum().item() if prev_bank_mask.any() else 0.0
                                ...
                                line = (
                                    f"[ATTN_MASS] "
                                    f"step={kv_cache['shared_dict'].get('current_timestep_index', -1)} "
                        ...
                        b_trg_out = attention(b_query.unsqueeze(0), b_trg_key.unsqueeze(0), b_trg_value.unsqueeze(0))

                        if (
                            (bank_aux_alpha > 0 or bank_gate_alpha > 0)
                            and context_prev_bank_mask.shape[0] == trg_prev_key.shape[1]
                            and context_prev_bank_mask.any()
                        ):
                            bank_slot_mask = context_prev_bank_mask
                            b_bank_key = trg_prev_key[b_idx][bank_slot_mask]
                            b_bank_value = trg_prev_value[b_idx][bank_slot_mask]

                            if bank_topk > 0 and b_bank_key.shape[0] > bank_topk:
                                q_flat = b_query.flatten(1).float()
                                k_flat = b_bank_key.flatten(1).float()
                                scores = torch.matmul(
                                    q_flat,
                                    k_flat.transpose(0, 1),
                                ) / (q_flat.shape[-1] ** 0.5)
                                topk_idx = scores.topk(bank_topk, dim=-1).indices

                                selected_key = b_bank_key[topk_idx]
                                selected_value = b_bank_value[topk_idx]
                                b_bank_out = attention(
                                    b_query.unsqueeze(1),
                                    selected_key,
                                    selected_value,
                                ).squeeze(1)
                            elif b_bank_key.shape[0] > 0:
                                b_bank_out = attention(
                                    b_query.unsqueeze(0),
                                    b_bank_key.unsqueeze(0),
                                    b_bank_value.unsqueeze(0),
                                )

                                if bank_gate_alpha > 0:
                                    diff = (b_bank_out - b_trg_out).abs().mean(dim=-1, keepdim=True)
                                    gate = torch.exp(-diff / max(bank_gate_tau, 1e-6)).clamp(0, 1)

                                    gate_log = kv_cache["shared_dict"].get("bank_gate_log", None)
                                    step_idx = kv_cache["shared_dict"].get("current_timestep_index", -1)
                                    total_step = kv_cache["shared_dict"].get("total_timestep", -1)
                                    if gate_log is not None and step_idx in {0, total_step // 2, total_step - 1}:
                                        with open(gate_log, "a") as f:
                                            f.write(
                                                f"[BANK_GATE] frame_start={current_start // 1560} "
                                                f"step={step_idx} "
                                                f"gate_mean={gate.mean().item():.6f} "
                                                f"gate_max={gate.max().item():.6f} "
                                                f"diff_mean={diff.mean().item():.6f} "
                                                f"bank_tokens={b_bank_key.shape[0]}\\n"
                                            )

                                    b_trg_out = b_trg_out + bank_gate_alpha * gate * (b_bank_out - b_trg_out)
                                elif bank_aux_alpha > 0:
                                    b_trg_out = b_trg_out * (1 - bank_aux_alpha) + b_bank_out * bank_aux_alpha

                        x_list.append(b_trg_out)
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        # Tag each self-attn with its layer index so the (optional) ΔKV
        # transport probe (Block 1) can key dumped/injected capsules by layer.
        ...
        for block_index, (current_end, local_end_index, update_info) in cache_update_infos:
            if update_info is not None:
                ...
                if update_info["action"] == "roll_and_insert":
                    ...
                    new_block_ids = bank.get("block_ids_new", None)
                    ...
                    if write_end_index > write_start_index and new_k.shape[1] == (write_end_index - write_start_index):
                        ...
                        if update_bank:
                            ...
                            if new_block_ids is not None:
                                new_block_id = max(current_end_bank // 1560 - 1, 0)
                                bank["block_ids_new"].fill_(new_block_id)
                            if "k_new_pending" in bank:
                                bank["k_new_pending"].fill_(True)
    ...
    def _apply_cache_updates_before(self, kv_bank, crossattn_cache):
        ...
        for block_index, block in enumerate(self.blocks):
            ...
            # Skip if the pending `k_new` was already materialized for this
            # block. Banks without the flag (native MemFlow inference/training,
            # which materializes exactly once per block) keep the old behavior.
            pending = bank.get("k_new_pending", None)
            if pending is not None and not bool(pending.item()):
                continue
            if write_end_index_bank >= 1560:
                ...
                if new_block_ids is not None:
                    new_block_ids = new_block_ids.clone()
                with torch.no_grad():
                    if write_end_index_bank <= bank["k"].shape[1]:
                        ...
                        if new_block_ids is not None and "block_ids" in bank:
                            bank["block_ids"][:, write_start_index_bank:write_end_index_bank] = new_block_ids
                    else:
                        ...
                        if "block_ids" in bank and "block_ids" in new_compressed_kv_cache:
                            bank["block_ids"][:,:] = new_compressed_kv_cache["block_ids"]
    ...
    def compress_kv_bank(
        ...
        new_block_ids: torch.Tensor = None,
        ...
        hist_block_ids = kv_cache.get('block_ids', None)
        if hist_block_ids is not None:
            hist_block_ids = hist_block_ids.clone()
        ...
        if hist_block_ids is not None and new_block_ids is not None:
            hist_id_blocks = hist_block_ids.view(B, num_hist_blocks, tokens_per_block)
        ...
        if hist_block_ids is not None and new_block_ids is not None:
            expanded_id_indices = topk_indices.view(B, k_to_select, 1).expand(-1, -1, tokens_per_block)
            salient_id_blocks = torch.gather(hist_id_blocks, 1, expanded_id_indices)
        ...
        if hist_block_ids is not None and new_block_ids is not None:
            new_id_blocks = new_block_ids.view(B, num_new_blocks, tokens_per_block)
        ...
        if hist_block_ids is not None and new_block_ids is not None:
            final_id_blocks = torch.cat([salient_id_blocks, new_id_blocks], dim=1)
        ...
        if hist_block_ids is not None and new_block_ids is not None:
            result["block_ids"] = final_id_blocks.reshape(B, final_num_tokens)
        return result
